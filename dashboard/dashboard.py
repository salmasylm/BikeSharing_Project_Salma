...
width = 0.35
bars1 = ax.bar(x - width/2, casual_register_df["total_casual"],     width, label="Casual",     color="#FF6347", zorder=3)
bars2 = ax.bar(x + width/2, casual_register_df["total_registered"], width, label="Registered", color="#4682B4", zorder=3)
ax.set_xticks(x)
ax.set_xticklabels(casual_register_df["yr"], fontsize=11)
ax.set_xlabel("Tahun", fontsize=11)
ax.set_ylabel("Jumlah", fontsize=11)
ax.set_title("Total Pengguna Casual vs Registered per Tahun", fontsize=13, fontweight="bold")
ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda val, _: f"{val:,.0f}"))
ax.legend(framealpha=0.8)
add_bar_labels(ax, bars1, color="#CC3300")
add_bar_labels(ax, bars2, color="#1a5276")
plt.tight_layout()
st.pyplot(fig)
